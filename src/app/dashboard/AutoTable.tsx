...
import { addAuto, deleteAuto, updateAuto } from '../services/AutoService'
import { AutoAddEditModal } from './AutoAddEditModal'

interface AutoTableProps {
  initialAutos: AutoDTO[]
  availableYears: number[]
}

export function AutoTable({ initialAutos, availableYears }: AutoTableProps) {
  const [autos, setAutos] = useState<AutoDTO[]>(initialAutos)
  ...
  const handleAddEditAuto = async (auto: AutoDTO) => {
    try {
      ...
      if (result.success) {
        ...
        setAutos((prev) => [...prev, auto]) // Optimistic update
      } else {
  ...
  const handleDeleteAuto = async (auto: AutoDTO) => {
    try {
      ...
      if (result.success) {
        ...
        setAutos((prev) => prev.filter((a) => a.id !== auto.id)) // Optimistic update
      } else {
        alert(`Error: ${result.message}`)
      }
    } catch (error) {
